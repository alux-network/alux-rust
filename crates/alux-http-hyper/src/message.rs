...
pub(crate) async fn asked<Sent>(request: Request<Sent>) -> Result<DirectRequest, DirectResponse>
where
    ...
    Sent::Error: Display,
{
    ...
    let body = sent
        .collect()
        .await
        .map_err(|error| DirectResponse::from(DirectError::unreadable("body", &error.to_string())))?;

    Ok(asked.with_body(body.to_bytes().to_vec()))
}
